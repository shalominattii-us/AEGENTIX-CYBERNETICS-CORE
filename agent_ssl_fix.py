...
if test_status == 200:
    ...
    log("⚠️ Connection failed - check your API key")
    log("💡 Try getting a new key from Moltbook")
else:
    log(f"⚠️ Connection status: {test_status}")
...
while True:
    try:
        ...
        if status == 200:
            ...
            log(f"[{counter}] ⚠️ No response - retrying...")
        else:
