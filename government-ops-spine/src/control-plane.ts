...
import { CollaborationFlowService, type OrganizationContact, type ReviewDecisionInput } from './collaboration-flow.js';
...
export class ControlPlaneRuntime {
  private readonly reviews:ArtifactReviewService;private readonly releases:ReleasePolicyService;private readonly documents:ControlledDocumentRenderer;private readonly receipts:ExecutionReceiptService;private readonly gmail:GmailDraftIntegration;private readonly collaboration:CollaborationFlowService;
  constructor(private readonly pool:Pool,private readonly registry:CapabilityRegistry,private readonly indexer:CapabilityIndexingWorker,private readonly engineeringPrograms:PostgresEngineeringProgramService,private readonly gmailProvider:GmailDraftProvider=new DisabledGmailDraftProvider()){
    this.reviews=new ArtifactReviewService(pool);this.releases=new ReleasePolicyService(pool);this.documents=new ControlledDocumentRenderer(pool);this.receipts=new ExecutionReceiptService(pool);this.gmail=new GmailDraftIntegration(pool);this.collaboration=new CollaborationFlowService(pool);
  }
  async handle(request:ControlRequest):Promise<ControlResponse|undefined>{
    ...
    if(request.method==='GET'&&request.path==='/api/reviews/inbox')return{status:200,body:await this.collaboration.reviewInbox('pending')};
    if(request.method==='GET'&&request.path==='/api/organizations')return{status:200,body:await this.collaboration.listOrganizations()};
    if(request.method==='POST'&&request.path==='/api/organizations')return{status:201,body:await this.collaboration.upsertOrganization(request.body as OrganizationContact)};
    const decision=request.path.match(/^\/api\/reviews\/([^/]+)\/(approve|deny|request-changes)$/);
    if(request.method==='POST'&&decision){const body=request.body as Omit<ReviewDecisionInput,'decision'>;const verb=decision[2];const mapped=verb==='request-changes'?'request_changes':verb as 'approve'|'deny';return{status:200,body:await this.collaboration.decide(decodeURIComponent(decision[1]),{...body,decision:mapped})};}
    ...
    const packageRoute=request.path.match(/^\/api\/engineering-programs\/([^/]+)\/complete-package$/);
    if(request.method==='POST'&&packageRoute){const b=request.body as {opportunityId:string;organizationId:string;manifestId:string;subject?:string;body?:string};return{status:201,body:await this.collaboration.assembleCompletePackage({...b,programId:decodeURIComponent(packageRoute[1])})};}
    const getPackage=request.path.match(/^\/api\/complete-packages\/([^/]+)$/);
    if(request.method==='GET'&&getPackage){const found=await this.collaboration.getPackage(decodeURIComponent(getPackage[1]));return found?{status:200,body:found}:{status:404,body:{error:'Complete package not found'}};}
    const packageGmail=request.path.match(/^\/api\/complete-packages\/([^/]+)\/gmail-draft$/);
    if(request.method==='POST'&&packageGmail){const mailbox=(request.body as {mailbox:string}).mailbox;return{status:201,body:await this.gmail.createFromApprovedPackage(decodeURIComponent(packageGmail[1]),mailbox,this.gmailProvider)};}
    ...
    const approve=request.path.match(/^\/api\/correspondence\/([^/]+)\/approve$/);if(request.method==='POST'&&approve)return{status:200,body:await this.approve(decodeURIComponent(approve[1]),request.body as ApprovalReceipt)};
    const gmailDraft=request.path.match(/^\/api\/correspondence\/([^/]+)\/gmail-draft$/);if(request.method==='POST'&&gmailDraft)return{status:201,body:await this.gmail.createReviewableDraft(decodeURIComponent(gmailDraft[1]),this.gmailProvider)};
    const get=request.path.match(/^\/api\/correspondence\/([^/]+)$/);if(request.method==='GET'&&get){const row=await this.pool.query('select * from correspondence_drafts where id=$1',[decodeURIComponent(get[1])]);return row.rowCount?{status:200,body:row.rows[0]}:{status:404,body:{error:'Correspondence draft not found'}};}
    return undefined;
  }
  private async createDraft(input:Omit<CorrespondenceDraft,'id'|'createdAt'|'status'>){if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(input.mailbox.address))throw new Error('A valid organizational mailbox is required.');const id=`CORR-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,createdAt=new Date().toISOString();await this.pool.query(`insert into correspondence_drafts(id,purpose,mailbox_json,to_json,cc_json,subject,body,attachments_json,opportunity_id,status,created_at) values($1,$2,$3,$4,$5,$6,$7,$8,$9,'draft',$10)`,[id,input.purpose,input.mailbox,input.to,input.cc,input.subject,input.body,input.attachments,input.opportunityId??null,createdAt]);await this.collaboration.enqueue({itemType:'correspondence_review',aggregateId:id,opportunityId:input.opportunityId,title:`Review correspondence: ${input.subject}`,summary:`Recipients: ${input.to.join(', ')}; attachments: ${input.attachments.length}`,requiredRole:'executive',payload:{correspondenceId:id,subject:input.subject,to:input.to,cc:input.cc,attachments:input.attachments}});await this.receipts.record('correspondence.draft.created',id,{purpose:input.purpose,mailbox:input.mailbox.address});return{id,createdAt,status:'draft',...input};}
