...
export class CapabilityIndexingWorker {
  ...
  async runOnce():Promise<number>{
    ...
    try {
      let count=0;
      for(const collector of this.collectors){for(const asset of await collector.collect()){await this.registry.upsert(asset);count++;}}
      this.health.lastAssetCount=count; this.health.lastCompletedAt=new Date().toISOString(); return count;
